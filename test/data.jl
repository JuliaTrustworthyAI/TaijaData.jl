...
@testset "Data loading tests" begin
    @testset "load_tabular_data tests" begin
        ...
        data = TaijaData.load_tabular_data_raw()
        @test all(k in keys(data) for k in keys(TaijaData.data_catalogue_raw[:tabular]))

        # Test dropping a specific dataset
        data = TaijaData.load_tabular_data_raw(; drop=:california_housing)
        @test !haskey(data, :california_housing)

        # Test loading with specified number of samples
        data = TaijaData.load_tabular_data_raw(1000)
    ...
    @testset "load_synthetic_data tests" begin
            # Test dropping a specific dataset
            data = TaijaData.load_synthetic_data_raw(; drop=:linearly_separable)
            @test !haskey(data, :linearly_separable)
    
            # Test loading with specified number of samples
            data = TaijaData.load_synthetic_data_raw(500)
            for dataset in values(data)
